...
class Actor:
    ...
    def sendDatatoBuyer(self, _buyerHost, _buyerPort, _AID, _SID, _filename):
	...
	def confirmDelivery(self, _AID, _SID):
        ...
        username = self.profile['UserName']
        registerDelivery_tx = RegisterSc.functions.registerDelivery(_AID, _SID).buildTransaction(
        {
            'from': self.profile['address'],
            'nonce': web3.eth.getTransactionCount(self.profile['address']),
            'chainId': web3.eth.chain_id,
        ...
        tx_create = web3.eth.account.signTransaction(registerDelivery_tx, self.profile['private_key'])
        tx_hash = web3.eth.sendRawTransaction(tx_create.rawTransaction)
        tx_receipt = web3.eth.waitForTransactionReceipt(tx_hash)
