...
class OperationBase extends WorkloadModuleBase {
    ...
    async initializeWorkloadModule(workerIndex, totalWorkers, roundIndex, roundArguments, sutAdapter, sutContext) {
        ...
        this.assertSetting('numberOfTrails');
        this.assertSetting('Marketplace');
        this.assertSetting('ratio');
        this.assertSetting('trackerScAddress');

        this.assertSetting('paymentAmount');
        this.trackerScAddress = this.roundArguments.trackerScAddress;
        this.ratio = this.roundArguments.ratio;
        this.numberOfTrails = this.roundArguments.numberOfTrails;
        this.createTrails = this.roundArguments.createTrails;
        this.Marketplace = this.roundArguments.Marketplace;
        this.paymentAmount = this.roundArguments.paymentAmount
    ...
    _createEthereumConnectorRequest(operation, args) {
        const validateTrail = operation === 'validateTrail';
        return {
            contract: 'NotarySc',
            verb: operation,
            args: Object.keys(args).map(k => args[k]),
            readOnly: validateTrail
        };
    }
}
