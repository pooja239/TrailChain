 ...
 */
 ...
 contract PaymentSc {
    ...
    function paymentShare(bytes32[] memory _actorID, uint[] memory _share, bytes32 _sender) public {
        uint len = _actorID.length;
        for(uint i=0; i<len; i++){
            address Sender = AccountID[_sender];
            address Recipient = AccountID[_actorID[i]];
            transferFrom(Sender, Recipient, _share[i]);
        }
    }
